...
var (
	...
	TeleopVisualizer = family.WithModel("teleop-visualizer")
)
...
// false positives from smoothing lag).
...
func SmoothPose(state *SmoothState, rawPos [3]float64, rawQuat mgl64.Quat, alpha float64) (Pose, *SmoothState) {
	...
	if state == nil || alpha >= 1.0 {
		...
		return Pose{X: rawPos[0], Y: rawPos[1], Z: rawPos[2], OX: ox, OY: oy, OZ: oz, ThetaDeg: th},
			&SmoothState{Pos: rawPos, Quat: rawQuat, RawPos: rawPos, RawQuat: rawQuat}
	}
	...
	return Pose{X: smoothPos[0], Y: smoothPos[1], Z: smoothPos[2], OX: ox, OY: oy, OZ: oz, ThetaDeg: th},
		&SmoothState{Pos: smoothPos, Quat: smoothQuat, RawPos: rawPos, RawQuat: rawQuat}
}
...
func IsOutlier(prev *SmoothState, rawPos [3]float64, rawQuat mgl64.Quat, maxPosMM, maxRotDeg float64) bool {
	...
	// Compare raw-to-raw (not raw-to-smoothed) to avoid false positives
	// from smoothing lag: the smoothed position trails the raw position,
	// and the growing gap would trigger cascading outlier rejections.
	dx := rawPos[0] - prev.RawPos[0]
	dy := rawPos[1] - prev.RawPos[1]
	dz := rawPos[2] - prev.RawPos[2]
	...
	dot := prev.RawQuat.Dot(rawQuat.Normalize())
	if dot < 0 {
		dot = -dot
	}
	if dot > 1 {
