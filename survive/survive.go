...
// vr_cal_status writes a human-readable calibration status string into buf.
// Returns the number of characters written (excluding null terminator).
static int vr_cal_status(char *buf, int bufLen) {
    if (!gCtx || !buf || bufLen <= 0) return 0;
    SurviveContext *ctx = survive_simple_get_ctx(gCtx);
    if (!ctx) return 0;
    return survive_cal_get_status(ctx, buf, bufLen);
}
...
// CalStatus returns a human-readable calibration status string from libsurvive.
func CalStatus() string {
	var buf [512]C.char
	n := C.vr_cal_status(&buf[0], 512)
	if n <= 0 {
		return ""
	}
